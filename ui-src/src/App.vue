<template>
  <van-config-provider :theme="theme ? 'light' : 'dark'">
    <van-nav-bar title="Easytier For KSU" safe-area-inset-top fixed>
      <template #left>
        <van-space>
...
const initTheme = () => {
  ...
  if (!isEmpty(cacheTheme)) {
    ...
    localStorage.setItem('EasytierForKSU.theme', false)
  }
}
const initI18n = () => {
  const cacheLocale = localStorage.getItem('EasytierForKSU.locale')
  ...
  localStorage.setItem('EasytierForKSU.locale', locale)
}
